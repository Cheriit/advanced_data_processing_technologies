...
DELETE FROM USER_SDO_GEOM_METADATA WHERE 1=1;
------
--1B--
------
...
FROM MAJOR_CITIES
WHERE CITY_NAME != 'Warsaw' and SDO_NN(
    GEOM,
    (SELECT GEOM
    FROM MAJOR_CITIES
...
SELECT
    CITY_NAME AS MIASTO
FROM MAJOR_CITIES
WHERE CITY_NAME != 'Warsaw' and SDO_WITHIN_DISTANCE(
    GEOM,
    (SELECT GEOM
    FROM MAJOR_CITIES
    WHERE CITY_NAME = 'Warsaw')
...
------
SELECT
    CNTRY_NAME AS PANSTWO,
    SDO_NN_DISTANCE(1) AS ODL
FROM COUNTRY_BOUNDARIES
WHERE CNTRY_NAME NOT LIKE 'Poland' AND SDO_NN(
    GEOM,
...
WHERE A.CITY_NAME = B.CITY_NAME AND SDO_GEOM.SDO_DISTANCE(
